      ...
      case 1:
        if ((coverImageDM.url?.isNotEmpty ?? false) && (coverImageDM.imageId?.isNotEmpty ?? false)) {
          await DashboardHelpers.cropImageFromUrl(coverImageDM.url!, context).then(
            (filePath) => filePath.isNotEmpty && context.mounted
                ? context.read<BusinessBloc>().add(BusinessEvent.updatePicture(coverImageDM.imageId!, filePath))
                : null,
          );
        }
        break;
      case 2:
        context.read<BusinessBloc>().add(BusinessEvent.deleteCoverImageById(coverImageDM));
        break;
