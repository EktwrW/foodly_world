  ...
  Future<bool> _applyAction(
    ...
    return res.when(
      success: (r) {
        ...
        // La respuesta de la mutación trae TODO lo que la pantalla necesita:
        // la orden, los contadores de los chips y si sigue perteneciendo al
        // panel en vivo (be-foodly #148). Así que no hay que releer nada.
        //
        // Antes se releía la lista entera sólo para mover un cubo, y eso eran
        // DOS lecturas por acción —la local y la del evento de Pusher—,
        // medidas en producción. Al quitar la local, la fila se quedaba a la
        // vista hasta que llegara el evento: al cerrar una cuenta o al filtrar
        // por un chip, hasta 2 s mostrando algo que el backend ya no incluye.
        // Ahora es correcto en el acto.
        //
        // `stillInPanel` lo decide el BACKEND. El predicado de "está en el
        // panel" se corrigió tres veces en agosto de 2026; replicarlo aquí
        // sería mantener dos copias de algo que ya costó caro con una.
        final sigue = r.stillInPanel ?? true;
        final lista = [
          for (final o in state.orders)
            if (o.uuid != updated.uuid) o else if (sigue) updated,
        ];

        emit(state.copyWith(
          orders: lista,
          counts: r.panelCounts ?? state.counts,
          total: r.panelTotal ?? state.total,
          error: null,
        ));
        return true;
      },
