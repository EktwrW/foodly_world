...
class _ReservationMessagesSheetState extends State<_ReservationMessagesSheet> with WidgetsBindingObserver {
  ...
  Widget _buildMessageList() {
    ...
    if (_messages.isEmpty) {
      // La hoja la abren las DOS partes —comensal y negocio— asi que el texto
      // no puede nombrar a ninguna. Sin boton: el compositor esta justo debajo.
      return FadeIn(
        child: FoodlyEmptyView(
          title: S.current.noMessagesYet,
          subtitle: S.current.reservationMessagesEmptyBody,
          icon: const Icon(Bootstrap.chat_left, size: 40, color: FoodlyThemes.primaryFoodly),
        ),
      );
    }
