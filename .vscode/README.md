...
These back paid services — must rotate periodically. Note que ya NO quedan claves de
generación de IA acá: se movieron al backend el 2026-08-18 (gap #10 cerrado en
`be-foodly/docs/security-hardening-backlog.md`).
...
Once env vars are set, reload VS Code (`Cmd+Shift+P` → "Developer: Reload Window") and start a Flutter launch configuration. In the Debug Console you should see the Flutter command with the keys interpolated. If you see literally `${env:GOOGLE_MAPS_API_KEY}` (or any other var) in the logged command, VS Code didn't have that var set.
...
Las claves de generación de IA ya no viven en el cliente: la generación de promos se
movió al backend el 2026-08-18 y sus credenciales están en GCP Secret Manager.
